...
export default class CanvasMsg extends Component {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!paginator.hasPrevPage) {
        ...
        paginator.prevPage().then(paginator => {
          ...
          this.setState({
            msgCache: update(this.state.msgCache, { [contact]: { $unshift: messages } }),
            pgtrCache: update(this.state.pgtrCache, { [contact]: { $set: paginator } })
          });
          resolve();
        });
      }
  ...
  componentWillUnmount() {
    ...
    Object.keys(this.msgAddedHandlerTracker).forEach(contact => {
      this.props.client.getChannelByUniqueName(contact)
        .then(channel => {
  ...
  msgAddedHandler = (contact, msg) => {
    //console.log("New msg for", contact, ":", msg);
    this.setState({
  ...
  fetchMsgsForContact(contact) {
    if (contact === "new") { return; }
    if (this.state.msgCache[contact] === undefined && !this.fetchTracker[contact]) {
      ...
      this.props.client.getChannelByUniqueName(contact).then(channel => {
        ...
        channel.getMessages(msgsPerPage).then((paginator) => {
          ...
          this.setState({
            msgCache: update(this.state.msgCache, { [contact]: { $set: messages } }),
            pgtrCache: update(this.state.pgtrCache, { [contact]: { $set: paginator } })
          });
        });
  ...
  updateNewPhoneNumber(e) {
    ...
    } else if (e === "") { // for resets after sending message in MsgComposer
